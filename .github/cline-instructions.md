# AI Copilot 使用与编写指南（简版） - Cline (v2025-10-14)
...
Cline 是一个 VSCode 扩展，支持在编辑器内进行 AI 辅助开发。使用时请注意：

1. **任务模式**: Cline 擅长接收明确的任务指令，在开始前请详细描述需求
2. **文件操作**: Cline 会展示文件变更预览，请仔细审查后再批准
3. **测试优先**: 对于重要变更，建议先编写测试用例，再实现功能
4. **增量开发**: 将大任务分解为小步骤，每步完成后验证
5. **构建验证**: 每次变更后及时运行构建和测试命令确保正确性
