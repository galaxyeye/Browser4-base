...
> IMPORTANT: The client side commands defined in `commands.ts` are the source of truth for what tools are expected to
> be available. This task focuses on ensuring that all those commands are fully supported and functional end-to-end,
> from the CLI declaration down to the WebDriver implementation.
