...
/**
 * Production-grade elementFromPoint with iframe + open shadow DOM support
 *
 * @param {number} x - viewport x
 * @param {number} y - viewport y
 * @param {Document | ShadowRoot} root
 * @param {HTMLIFrameElement[]} iframeChain
 * @param {ShadowRoot[]} shadowChain
 * @returns {Object}
 */
__pulsar_utils__.elementFromPointDeep = function(
    x,
    y,
    root = document,
    iframeChain = [],
    ...
    try {
        // ShadowRoot doesn't have elementsFromPoint in most browsers; use its host's document.
        if (root && root.nodeType === 11 /* DOCUMENT_FRAGMENT_NODE */ && root.host) {
            // Use viewport coords for the host's document and narrow results by containment.
            const doc = root.host.ownerDocument || document;
            const all = doc.elementsFromPoint(x, y) || [];
            elements = all.filter(e => e === root.host || (root.host.contains && root.host.contains(e)));
            if (elements.length === 0) {
                // Include host as a candidate so we can still descend into its shadow root.
                elements = [root.host];
            }
        } else {
    ...
    for (const el of elements) {
        // Skip non-elements or invisible hits.
        ...
        try {
            ...
            // Ignore styling failures and continue.
        }
        ...
        if (el.tagName === 'IFRAME') {
            ...
            try {
                ...
                // cross-origin iframe，无法深入
                return {
                    target: iframe,
                    path: elements,
                    iframeChain: iframeChain.concat(iframe),
                    shadowChain
                };
            }
            ...
            const res = __pulsar_utils__.elementFromPointDeep(
                x - rect.left,
                y - rect.top,
                doc,
                iframeChain.concat(iframe),
                shadowChain
            );
        ...
        if (el.shadowRoot && el.shadowRoot.mode === 'open') {
            // When we're already inside iframes, x/y are in the current document's viewport coords.
            // Translate into the shadow host's local box before searching inside shadow.
            ...
            const res = __pulsar_utils__.elementFromPointDeep(
                ...
                el.shadowRoot,
                iframeChain,
                shadowChain.concat(el.shadowRoot)
            );
