...
class OpenApiContractTest {
    ...
    private fun extractLegacyAntPatterns(info: RequestMappingInfo): Set<String> {
        return runCatching {
            val method =
                info.javaClass.methods.firstOrNull { it.name == "getPatternsCondition" && it.parameterCount == 0 }
                    ?: return@runCatching emptySet<String>()

            val condition = method.invoke(info) ?: return@runCatching emptySet<String>()
            val patternsMethod =
                condition.javaClass.methods.firstOrNull { it.name == "getPatterns" && it.parameterCount == 0 }
                    ?: return@runCatching emptySet<String>()
